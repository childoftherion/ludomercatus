...
- **[Bun](https://bun.sh)** (recent 1.x). The project is built and run with Bun.
...
**What this does:** `bun --hot` watches the entry graph and reloads the server when dependencies change; `run dev` still resolves to `bun run index.ts` via `package.json`.
...
If you **already cloned** the repo yourself, skip the one-liners and use [Installation](#installation) and [Running the game](#running-the-game).
