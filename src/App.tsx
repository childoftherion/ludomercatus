    ...
    if (phase !== "setup" && myPlayerIndex === -1) {
      ...
      if (reclaimablePlayers.length === 1 && reclaimablePlayers[0]) {
        ...
        if (nextPlayer) {
          ...
          useGameStore
            .getState()
            .assignPlayer(nextPlayer.index, clientId, isMobile)
        }
      }
    }
  ...
  return (
    <ErrorBoundary>
      ...
      >
        ...
        {(() => {
          const myPlayer =
            myPlayerIndex !== undefined && myPlayerIndex >= 0
              ? players[myPlayerIndex]
              : null
          if (!myPlayer?.inChapter11) return null
          const progress =
            myPlayer.cash >= myPlayer.chapter11DebtTarget
              ? 100
              : Math.round(
                  (myPlayer.cash / Math.max(1, myPlayer.chapter11DebtTarget)) *
                    100,
                )
          return (
            <div
