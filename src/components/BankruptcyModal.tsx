...
export const BankruptcyModal: React.FC = () => {
  ...
  const creditor =
    creditorIndex !== undefined ? players[creditorIndex] : undefined
  ...
  const totalAssetValue = playerProperties.reduce(
    (sum: number, prop: Property) => {
      // Property value (adjusted for market fluctuations)
      const propValue = getCurrentPropertyPrice(useGameStore.getState(), prop)
      ...
      const buildingValue = Math.floor(
        ((prop.houses + (prop.hotel ? 1 : 0)) * (prop.buildingCost ?? 0)) / 2,
      )
  ...
  return (
    <AnimatePresence>
      <motion.div
        role="dialog"
        aria-label={`Bankruptcy decision for ${player.name}`}
        ref={modalRef}
        ...
        <div style={{ padding: '20px' }}>
          ...
          >
            ...
            >
              ...
              >
                <li>
                  All your properties go to {creditor?.name ?? 'the bank'}
                </li>
              ...
              <button
                onClick={() => {
                  if (isMyBankruptcy) {
                    console.log(
                      '[BankruptcyModal] Declaring bankruptcy via declineRestructuring',
                    )
                ...
                style={{
                  width: '100%',
                  padding: '14px',
                  backgroundColor: isMyBankruptcy
                    ? 'rgba(239, 68, 68, 0.2)'
