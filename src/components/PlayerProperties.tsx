...
export const PlayerPropertiesPanel = ({
  playerIndex,
  myPlayerIndex,
  ...
  const shouldHideWealth =
    settings?.hideOpponentWealth && !isYou && !player?.bankrupt
  const shouldHideProperties =
    settings?.hideOpponentProperties && !isYou && !player?.bankrupt
  ...
  const economyState = React.useMemo(
    () => ({ players, spaces, activeEconomicEvents }) as const,
    [players, spaces, activeEconomicEvents],
  )
  ...
  React.useEffect(() => {
    if (isYou && selectedProperty && player) {
      const buildDebug = {
        ...
        hasMonopoly: selectedProperty.colorGroup
          ? hasMonopoly(playerIndex, selectedProperty.colorGroup)
          : false,
  ...
  return (
    ...
    >
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {settings?.enablePropertyValueFluctuation &&
            ...
            >
              ...
              {canBuild &&
                ...
                (() => {
                  ...
                  const canAfford =
                    player.cash >= (selectedProperty.buildingCost ?? 0)
              ...
              <button
                onClick={() =>
                  selectedProperty.mortgaged
                    ? unmortgageProperty(selectedProperty.id)
                    : mortgageProperty(selectedProperty.id)
                }
                disabled={
                  !selectedProperty.mortgaged &&
                  (selectedProperty.houses > 0 || selectedProperty.hotel)
              ...
              {settings?.enablePropertyInsurance &&
                !selectedProperty.mortgaged &&
                (() => {
                  const currentPrice = getCurrentPropertyPrice(
                    economyState as any,
                    selectedProperty,
                  )
                  const insuranceCost = Math.ceil(
                    currentPrice * (settings?.insuranceCostPercent ?? 0.05),
                  )
                  return (
                    <button
                      onClick={() =>
                        buyPropertyInsurance(selectedProperty.id, playerIndex)
                      }
                      disabled={
      ...
      {settings?.enableRentNegotiation &&
                        ...
                        {isYou && (
                          <button
                            onClick={() => {
                              ...
                              setIOUPayAmount(
                                Math.min(player.cash, iou.currentAmount),
                              )
                            }}
                            style={{
                              background: '#22c55e',
        ...
        {selectedIOUId !== null && isYou && (
          ...
          >
            ...
            >
              ...
              {(() => {
                const iou = player.iousPayable?.find(
                  (i: IOU) => i.id === selectedIOUId,
                )
                if (!iou) return null
                const creditor = players[iou.creditorId]
                return (
