...
export const PropertyDetailsModal: React.FC<PropertyDetailsModalProps> = ({
  ...
  return (
    <AnimatePresence>
      <motion.div
        role="dialog"
        aria-label={`Property details: ${property.name}`}
        ...
        >
          ...
          >
            <div>
              ...
              {getCurrentPropertyPrice(useGameStore.getState(), property) !== property.price && (
                ...
                property.valueMultiplier !== 1.0 && (
                  <span
                    style={{
                      marginLeft: '8px',
                      color:
                        property.valueMultiplier > 1 ? '#2e7d32' : '#d32f2f',
                      fontWeight: 'bold',
                      fontSize: '12px',
                    }}
                  >
            ...
            {property.type === 'utility' && (
              <div>
                ...
                >
                  ...
                  >
                    Unowned utilities: landing fee ({(4 * (property.valueMultiplier ?? 1)).toFixed(1)}×
                    dice, before events) is paid to the Jackpot, then you may
                    purchase the deed.
                  </div>
                  {property.valueMultiplier !== 1.0 && (
                    <div
            ...
            <div>
              <strong>Mortgage Value:</strong> £{property.mortgageValue}
            </div>
            ...
            {ownerName ? (
              ...
              <div>
                <strong>Owner:</strong>{' '}
                <span style={{ color: '#666' }}>Unowned</span>
              </div>
            )}
            ...
            {property.houses > 0 && (
              <div>
                <strong>Houses:</strong> {property.houses}
              </div>
            )}

            {property.hotel && (
              <div>
                <strong>Hotel:</strong> Yes
              </div>
            )}

            {enableHousingScarcity &&
              property.buildingCost &&
              property.buildingCost > 0 && (
                <div
                  style={{
                    marginTop: '8px',
                    padding: '8px',
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{
                        color: houseSupply === 0 ? '#d32f2f' : '#2e7d32',
                        fontWeight: 'bold',
                      }}
                    >
                  ...
                  >
                    ...
                    <span
                      style={{
                        color: hotelSupply === 0 ? '#d32f2f' : '#2e7d32',
                        fontWeight: 'bold',
                      }}
                    >
            ...
            {property.isInsured && property.insurancePaidUntilRound > 0 && (
              <div style={{
                ...
                <span style={{ fontSize: '11px', fontWeight: 'normal', color: '#666' }}>
                  Until round {property.insurancePaidUntilRound}
                </span>
              </div>
            )}
            {!property.isInsured && property.buildingCost && property.buildingCost > 0 && (property.houses > 0 || property.hotel) && (
              <div style={{
                fontSize: '11px',
                color: '#888',
                fontStyle: 'italic',
              }}>
                Not insured — developments at risk from market events
              </div>
            )}

            {property.valueMultiplier !== 1.0 && (
              <div>
                <strong>Value Multiplier:</strong>{' '}
                {property.valueMultiplier > 1 ? '+' : ''}
