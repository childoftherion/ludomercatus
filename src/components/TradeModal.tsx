...
export const TradeModal: React.FC = () => {
  ...
  const [isCreatingCounterOffer, setIsCreatingCounterOffer] =
    React.useState(false)
  const [counterOfferDraft, setCounterOfferDraft] =
    React.useState<TradeOffer | null>(null)
  ...
  const canMakeCounterOffer =
    isReceiver && !hasCounterOffer && status === 'pending'
  ...
  const handleCounterOfferCashChange = (
    amount: number,
    side: 'from' | 'to',
  ) => {
    if (!counterOfferDraft) return
    ...
    if (side === 'from') {
      ...
      newCounterOffer.cashRequested = Math.max(
        0,
        Math.min(amount, fromPlayer.cash),
      )
    }
    setCounterOfferDraft(newCounterOffer)
  }

  const handleCounterOfferToggleProperty = (
    propId: number,
    side: 'from' | 'to',
  ) => {
    if (!counterOfferDraft) return
    const newCounterOffer = { ...counterOfferDraft }
    if (side === 'from') {
      newCounterOffer.propertiesOffered =
        counterOfferDraft.propertiesOffered.includes(propId)
          ? counterOfferDraft.propertiesOffered.filter(id => id !== propId)
          : [...counterOfferDraft.propertiesOffered, propId]
    } else {
      newCounterOffer.propertiesRequested =
        counterOfferDraft.propertiesRequested.includes(propId)
            ...
            >
              <div
                style={{
                  padding: '8px',
                  borderRadius: '8px',
                  background: fromPlayer.color + '22',
                  borderLeft: `3px solid ${fromPlayer.color}`,
                }}
              >
              ...
              {status === 'draft' ? (
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      onChange={e =>
                        handleCashChange(parseInt(e.target.value) || 0, 'from')
                      }
                      style={{
                        width: '100%',
                        padding: '6px',
                      ...
                      {fromPlayerOwnedProps.map((prop: Property) => (
                        <div
                          ...
                          style={{
                            padding: '4px 8px',
                            fontSize: '11px',
                            background: offer.propertiesOffered.includes(
                              prop.id,
                            )
                              ? '#2E8B57'
                              : '#222',
                        ...
                        >
                          {prop.name}
                          {offer.propertiesOffered.includes(prop.id) && (
                            <span>✓</span>
                          )}
                        </div>
                      ))}
                      {fromPlayerOwnedProps.length === 0 && (
                        <div
                          style={{
                            fontSize: '11px',
                            opacity: 0.5,
                ...
                >
                  ...
                  {offer.cashOffered === 0 &&
                    offer.propertiesOffered.length === 0 && (
            ...
            >
              <div
                style={{
                  padding: '8px',
                  borderRadius: '8px',
                  background: toPlayer.color + '22',
                  borderRight: `3px solid ${toPlayer.color}`,
                  textAlign: 'right',
                }}
              >
                <div style={{ fontWeight: 'bold', fontSize: '13px' }}>
              ...
              {status === 'draft' ? (
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      onChange={e =>
                        handleCashChange(parseInt(e.target.value) || 0, 'to')
                      }
                      style={{
                        width: '100%',
                        padding: '6px',
                      ...
                      {toPlayerOwnedProps.map((prop: Property) => (
                        <div
                          ...
                          style={{
                            padding: '4px 8px',
                            fontSize: '11px',
                            background: offer.propertiesRequested.includes(
                              prop.id,
                            )
                              ? '#2E8B57'
                              : '#222',
                        ...
                        >
                          {prop.name}
                          {offer.propertiesRequested.includes(prop.id) && (
                            <span>✓</span>
                          )}
                        </div>
                      ))}
                      {toPlayerOwnedProps.length === 0 && (
                        <div
                          style={{
                            fontSize: '11px',
                            opacity: 0.5,
                ...
                >
                  ...
                  {offer.propertiesRequested.map(id => (
                    <p key={id} style={{ margin: '2px 0', fontSize: '12px' }}>
                      • {getPropertyName(id)}
                    </p>
                  ))}
                  {offer.cashRequested === 0 &&
            ...
            {isCreatingCounterOffer && counterOfferDraft && (
              <div style={{ width: '100%' }}>
                ...
                >
                  ...
                  >
                    You are now proposing a trade where {toPlayer.name} (you)
                    offers something to {fromPlayer.name}
                  </p>
                      ...
                      >
                        {toPlayerOwnedProps.map((prop: Property) => (
                          <div
                            key={prop.id}
                            onClick={() =>
                              handleCounterOfferToggleProperty(prop.id, 'from')
                            }
                            style={{
                              padding: '3px 6px',
                              fontSize: '10px',
                              background:
                                counterOfferDraft.propertiesOffered.includes(
                                  prop.id,
                                )
                                  ? '#2E8B57'
                                  : '#222',
                      ...
                      >
                        {fromPlayerOwnedProps.map((prop: Property) => (
                          <div
                            key={prop.id}
                            onClick={() =>
                              handleCounterOfferToggleProperty(prop.id, 'to')
                            }
                            style={{
                              padding: '3px 6px',
                              fontSize: '10px',
                              background:
                                counterOfferDraft.propertiesRequested.includes(
                                  prop.id,
                                )
                                  ? '#2E8B57'
                                  : '#222',
                ...
                {isInitiator ? (
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            {toPlayer.name} Offers:
                          </div>
                          {counterOfferData.cashOffered > 0 && (
                            <div style={{ fontSize: '11px' }}>
                              £{counterOfferData.cashOffered.toLocaleString()}
                            </div>
                          )}
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {counterOfferData.cashRequested > 0 && (
                            <div style={{ fontSize: '11px' }}>
                              £{counterOfferData.cashRequested.toLocaleString()}
                            </div>
                          )}
                  ...
                  <div style={{ textAlign: 'center', padding: '16px' }}>
                    <p style={{ color: '#FF9800' }}>
                      Waiting for {fromPlayer.name} to respond to your
                      counter-offer...
                    </p>
                    <motion.button
                      whileHover={{ scale: 1.05 }}
