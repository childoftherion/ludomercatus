...
export const executeAITurn = (state: GameState, actions: GameActions) => {
  ...
  if (state.phase === 'awaiting_debt_service' && state.pendingDebtService) {
    if (
      state.pendingDebtService.playerIndex === playerIndex &&
  ...
  if (
    state.settings?.enableBankLoans &&
    actions.takeLoan &&
  ...
  if (
    state.settings?.enablePropertyInsurance &&
    actions.buyPropertyInsurance &&
    player.cash > cashReserve * 1.5
  ) {
  ...
  if (
    state.phase === 'rolling' &&
    (!player.lastTradeTurn || state.turn - player.lastTradeTurn > 5)
  ) {
      ...
      if (ownedByAI.length > 0 && ownedByAI.length < groupProps.length) {
        ...
        if (owners.size === 1 && !owners.has(undefined)) {
          ...
          if (
            ...
            if (
              requestHistory?.lastOfferTurn !== undefined &&
...
export const executeAITradeResponse = (
  ...
  if (trade.status === 'counter_pending' && trade.counterOffer) {
    ...
    if (valueReceived >= valueGiven * threshold) {
