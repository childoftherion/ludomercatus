...
export class GameRoom implements GameActions {
  ...
  public addPlayer(name: string, token: string, clientId: string, isMobile?: boolean) {
  ...
  this.addLogEntry(`${uniqueName} joined the lobby!${isMobile ? " (Mobile)" : ""}`, "system");
}
  ...
  public assignPlayer(index: number, clientId: string) {
    ...
    console.log(`[GameRoom] Player ${index} (${player.name}) claimed by ${clientId}`);
  }
  ...
  public startAuction(propertyId: number) {
    ...
    const startingBid = Math.max(
      10,
      Math.floor((property?.price ?? 100) * 0.1),
    );
