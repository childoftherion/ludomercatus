...
export class GameRoom implements GameActions {
  ...
  public rollDice() {
    ...
    const nextConsecutiveDoubles = isDoubles
  ...
  public updateTradeOffer(offer: TradeOffer) {
    ...
    const rolesSwapped =
      offer.fromPlayer !== this.state.trade.offer.fromPlayer;
    this.setState({
      trade: {
        ...this.state.trade,
        offer,
  ...
  private validateTradeOffer(offer: TradeOffer): {
    ...
    const checkPropertyValid = (
      ...
      if (prop.colorGroup) {
        const groupProps = this.state.spaces.filter(
          (s): s is Property =>
            isProperty(s) && s.colorGroup === prop.colorGroup,
        );
        const hasBuildingsInGroup = groupProps.some(
          (p) => p.houses > 0 || p.hotel,
        );
    ...
    for (const propId of offer.propertiesOffered) {
      const result = checkPropertyValid(
        propId,
        offer.fromPlayer,
        fromPlayer.name,
      );
    ...
    for (const propId of offer.propertiesRequested) {
      const result = checkPropertyValid(propId, offer.toPlayer, toPlayer.name);
      if (!result.valid) return result;
    }
