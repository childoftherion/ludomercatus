...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  connect: () => {
    ...
    socket.onopen = () => {
      ...
      if ((get() as any)._pingInterval)
        clearInterval((get() as any)._pingInterval);
      (get() as any)._pingInterval = setInterval(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.type === "STATE_UPDATE") {
          ...
          console.log("[STORE] Room created, joining:", data.roomId);
  ...
  createRoom: (mode = "single", settings?: Partial<GameSettings>) => {
    socket?.send(JSON.stringify({ type: "CREATE_ROOM", mode, settings }));
  },
  createSinglePlayerGame: (
    names: string[],
    tokens: string[],
    ...
    socket?.send(
      JSON.stringify({
        ...
        playerData: { names, tokens, isAIFlags, aiDifficulties, clientIds },
        settings,
      }),
    );
  },
  ...
  addPlayer: (name: string, token: string) => {
    ...
    socket?.send(
      JSON.stringify({
        type: "ACTION",
        action: "addPlayer",
        payload: [name, token, clientId, isMobile],
      }),
    );
  },
  updatePlayer: (index, name, token) => {
    socket?.send(
      JSON.stringify({
        type: "ACTION",
        action: "updatePlayer",
        payload: [index, name, token],
      }),
    );
  },
  startGame: () => {
    socket?.send(
      JSON.stringify({ type: "ACTION", action: "startGame", payload: [] }),
    );
  },
  ...
  ...[
    ...
    "payDebtService",
    "handleForeclosureDecision",
    "appreciateColorGroup",
    "depreciateColorGroup",
    "assignPlayer",
  ].reduce((acc, action) => {
    acc[action] = (...args: any[]) => {
      if (socket && socket.readyState === WebSocket.OPEN) {
        try {
            ...
            if (typeof arg === "object" && arg !== null) {
              try {
                ...
                try {
                  ...
                  console.error(
                    `Failed to serialize arg for action ${action}, using null:`,
                    e2,
                  );
                  return null;
                }
          ...
          const message = JSON.stringify(
            { type: "ACTION", action, payload: cleanedPayload },
            finalReplacer,
          );
          socket.send(message);
        } catch (e) {
