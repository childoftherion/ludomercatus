...
export function validateBuilding(
  ...
  if (ownedByPlayer.length !== groupProperties.length) {
    return {
      valid: false,
      error: 'You must own all properties in this color group to build',
    }
  }
...
export function validateTradeOffer(
  ...
  for (const propId of offer.propertiesOffered) {
    ...
    if (!prop || prop.owner !== offer.fromPlayer) {
      return {
        valid: false,
        error: `You do not own property ${prop?.name || propId}`,
      }
    }
...
export function validateMortgage(
  ...
  const result = validatePropertyOwnership(
    state,
    propertyId,
    playerIndex,
    'mortgage',
  ...
  if (isUnmortgaging) {
    ...
    if (property.houses > 0 || property.hotel) {
      return {
        valid: false,
        error: 'Cannot mortgage properties with buildings',
      }
    }
  }
